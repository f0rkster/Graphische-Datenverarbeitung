...
namespace
{
    ...
    private:
        ...
        BHandle m_pWallMesh;        // A pointer to a YoshiX mesh, which represents a single quad.
        BHandle m_pWallTexture;     // A pointer to a YoshiX texture, which is part of the material covering the quad.
        BHandle m_pLeafMesh;        // A pointer to a YoshiX mesh, which represents a single quad.
        BHandle m_pLeafTexture;     // A pointer to a YoshiX texture, which is part of the material covering the quad.
...
namespace
{
    ...
    bool CApplication::InternOnCreateTextures()
    {
        ...
        CreateTexture("..\\data\\images\\leaf.dds", &m_pLeafTexture);
    ...
    bool CApplication::InternOnCreateMeshes()
    {
        ...
        CreateMesh(MeshInfo, &m_pWallMesh);

        MeshInfo.m_pTexture         = m_pLeafTexture;

        CreateMesh(MeshInfo, &m_pLeafMesh);
    ...
    bool CApplication::InternOnFrame()
    {
        ...
        // -----------------------------------------------------------------------------
        // Activate alpha blending. Since rendering of transparent surfaces cannot be
        // handled by the depth buffer we switch it off.
        // -----------------------------------------------------------------------------
        SetDepthTest(false);
